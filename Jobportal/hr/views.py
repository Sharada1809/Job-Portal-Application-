...
def hrHome(request):
    ...
    return render(request,'hrdash.html',{'jobposts':jobposts})
...
def hrCandidateDetails(request,id):
    if JobPost.objects.filter(id=id).exists():
        ...
        return render(request,'candidate.html',{'jobapplys':jobapplys,'jobpost':jobpost,'selectedCandidate':selectedCandidate})
    else:
...
def hrnavbar(request):
    return render(render,'hrnavbar.html')